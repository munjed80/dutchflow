...
## Food and shopping expansion

`groceries-and-quantities` teaches a half-kilogram request, counted items, price per kilogram versus the requested amount, and `nodig hebben` with `geen`. `clothes-and-sizes` teaches requesting a colour/size, asking to try a coat on, and explaining `te klein / te groot` before requesting a different size. Each lesson has eight phrases, four questions, six lexical notes and two self-review tasks. All prices are fictional teaching examples.
