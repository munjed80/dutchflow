...
test("the A1 map covers every published resource and preserves all original phrase identities and text", () => {
  ...
  assert.equal(lessons.flatMap((lesson) => lesson.phrases).length, 141);
  assert.equal(lessons.flatMap((lesson) => lesson.questions).length, 80);
  ...
  assert.equal(extensions.flatMap((item) => item.vocabulary).length, 42);
  assert.equal(extensions.flatMap((item) => item.tasks).length, 14);
});
